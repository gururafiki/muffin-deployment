...
begin
  select c.relkind into k from pg_class c join pg_namespace n on n.oid = c.relnamespace
   where n.nspname = 'market' and c.relname = 'pending_price_history';
  if k = 'm' then execute 'drop materialized view if exists market.pending_price_history cascade';
  elsif k = 'v' then execute 'drop view if exists market.pending_price_history cascade';
  end if;
end $$;
