-- SEC ASSIGNS EVERY REGISTRANT AN INDUSTRY, AND WE WERE STORING THE CODE WITHOUT THE MEANING.
--
-- `security-filing-history` writes `security.sic` and `sic_description` from the submissions
-- response (Apple: 3571, "Electronic Computers"). That is a real classification — assigned by SEC
-- from the registrant's own description of its business, and therefore an opinion that is neither
-- yfinance's nor a fund's — and until now it sat in two columns that nothing joined.
--
-- This makes it a proper taxonomy, so a security carries it the same way it carries every other
-- classification: a row in `security_taxonomy` with its own `source_code`.
--
-- THE CODES AND TITLES ARE SEC'S OWN, FETCHED FROM SEC, not transcribed. 444 of them, parsed
-- from the published SIC list. Authoring reference data from memory is how Taiwan once vanished
-- from this schema.
--
-- ONE LEVEL, DELIBERATELY. SIC is genuinely hierarchical — division, major group, industry group,
-- industry — but SEC publishes only the codes it ASSIGNS, with no names for the 2- and 3-digit
-- parents. Inventing those names would be authoring reference data from memory to make a tree look
-- complete. The 4-digit level is what SEC states and what a filer actually has.
--
-- PRIORITY 70, BELOW yfinance (100), so nothing that resolves a sector today changes. SIC is a
-- 1987 vocabulary and is most useful as a SECOND opinion beside a modern one — its value here is
-- that it disagrees, and that it exists for the 3,518 securities SEC knows about regardless of
-- whether a provider has ever answered for them.

insert into market.data_source (code, name, priority) values
  ('sic', 'SEC Standard Industrial Classification', 70)
...
insert into market.taxonomy (taxonomy_id, name, description) values
  ('sic', 'SIC', 'US Standard Industrial Classification, as SEC assigns it to a registrant')
on conflict (taxonomy_id) do update
  set name = excluded.name, description = excluded.description;

insert into market.taxonomy_node (taxonomy_id, code, name, level)
select 'sic', v.code, v.name, 1 from (values
...
on conflict (taxonomy_id, code) do update set name = excluded.name;

-- ── Linking a filer to its code ───────────────────────────────────────────────────────────────
--
-- Pure SQL, no provider, so it lives beside the other taxonomy derivations in
-- `derive-classifications` rather than becoming a resource of its own.
...
comment on function market.derive_sic_classification() is
  'Links each registrant to the SIC node SEC assigned it. Pure SQL. Deletes the link when the registrant''s SIC changes, because an upsert cannot retract.';

revoke all on function market.derive_sic_classification() from public;
grant execute on function market.derive_sic_classification() to service_role;
